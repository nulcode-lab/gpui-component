...
/// Linear blend of two colors; `factor` is the weight of `a` (0.95 = 95% a + 5% b).
/// Stand-in for the styled layer's `mix_oklab`, which base cannot reach.
fn blend_linear(a: Hsla, b: Hsla, factor: f32) -> Hsla {
    Hsla {
        h: a.h * factor + b.h * (1. - factor),
        s: a.s * factor + b.s * (1. - factor),
        l: a.l * factor + b.l * (1. - factor),
        a: a.a * factor + b.a * (1. - factor),
    }
}
...
impl<M: InputModeKind> Element for TextElement<M> {
    ...
    fn prepaint(
        ...
        let line_numbers = if state.mode.line_number() {
            ...
            let other_line_runs = vec![TextRun {
                len: line_number_len,
                font: style.font(),
                // 半透明让非当前行号与行号区底色混合，浅色/深色主题下都自动降低对比度
                color: state.editor_style.muted_foreground.opacity(0.7),
                background_color: None,
                underline: None,
                strikethrough: None,
            }];
    ...
    fn paint(
        ...
        if let Some(line_numbers) = prepaint.line_numbers.as_ref() {
            ...
            // Each item is the normal lines.
            let error_color = editor_style.diagnostics.error;
            ...
            // (95% border + 5% foreground, linear blend — mix_oklab lives in the styled layer).
            let separator_color = if cx.theme().appearance == crate::theme::ThemeAppearance::Dark {
                blend_linear(editor_style.border, editor_style.foreground, 0.95)
            } else {
                editor_style.border
            };
            let separator_x =
