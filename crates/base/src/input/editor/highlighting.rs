...
pub trait InputHighlighter {
    ...
    /// Whether `offset` sits inside a comment or string node of the syntax tree.
    /// Used by rainbow-bracket depth computation to skip brackets in literals.
    fn is_at_comment_or_string(&self, offset: usize) -> bool {
        let _ = offset;
        false
    }

    /// Whether `offset` sits inside a string node (quote autoclose decisions).
    /// Implementations may encode language heuristics (e.g. C++ char literals).
    fn is_in_string(&self, offset: usize) -> bool {
        let _ = offset;
        false
    }

    /// Returns a background-computable matched-bracket query for `offset`.
    /// The boxed closure must be `Send` and returns
    /// `(open_start, open_len, close_start, close_len)` byte offsets.
