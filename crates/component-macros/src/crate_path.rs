...
/// `gpui` fallback keeps git-fork consumers (algosoft) working.
pub(crate) fn gpui() -> syn::Result<TokenStream> {
    match crate_name("gpui-kit") {
        ...
        Err(kit_error) => match crate_name("gpui-pre") {
            Ok(found) => Ok(found_crate_path(found)),
            Err(gpui_error) => {
                crate_name("gpui").map(found_crate_path).map_err(|plain_error| {
                    syn::Error::new(
                        Span::call_site(),
                        format!(
                            "IntoPlot requires a direct dependency on `gpui-kit`, `gpui-pre` or \
                             `gpui`: gpui-kit lookup failed: {kit_error}; gpui-pre lookup \
                             failed: {gpui_error}; gpui lookup failed: {plain_error}"
                        ),
                    )
                })
            }
