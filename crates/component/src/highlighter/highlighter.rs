...
impl SyntaxHighlighter {
    ...
    pub fn bracket_pairs(&self, range: Range<usize>) -> Vec<(Range<usize>, Range<usize>)> {
        ...
        let Some(language) = &config.language else { return result };

        if config.brackets.is_empty() {
            return result;
        }

        let Ok(query) = Query::new(language, &config.brackets) else { return result };
...
fn innermost_bracket_pair_impl(
    ...
    let Some(language) = &config.language else { return None };

    if config.brackets.is_empty() {
        return None;
    }

    let Ok(query) = Query::new(language, &config.brackets) else { return None };
